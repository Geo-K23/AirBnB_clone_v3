...
class TestConsoleCommands(unittest.TestCase):
    ...
    @classmethod
    def setUpClass(cls):
        """Creating a command console to test with"""
        cls.cmdcon = HBNBCommand()

    def setUp(self):
        """This creates in memory a buffer to capture the stdout"""
        self.output = io.StringIO()

    def tearDown(self):
        """This closes the buffer in memory after test completes"""
        self.output.close()
    ...
    def test_do_create_db(self):
        """This tests the do_create method of console"""
        with redirect_stdout(self.output):
            self.cmdcon.onecmd('create')
            self.assertEqual(self.output.getvalue(),
                             "** class name missing **\n")
